...
impl WebBackend {
    ...
    fn build_search_url(&self, query: &str, offset: usize, count: usize) -> String {
        let encoded = urlencoding::encode(query);
        let start = offset.saturating_add(1);
        format!(
            "{}?mode=json&hoogle={}&start={}&count={}",
            self.base_url, encoded, start, count
        )
    }
}
...
impl HoogleBackend for WebBackend {
    async fn search(
        ...
        let url = self.build_search_url(query, offset, count);
...
mod tests {
    ...
    fn build_search_url_basic() {
        ...
        let url = backend.build_search_url("map", 0, 10);
        assert_eq!(
            url,
    ...
    fn build_search_url_special_chars() {
        let backend = WebBackend::new(&test_config()).unwrap();
        let url = backend.build_search_url("a -> b", 0, 5);
        assert!(url.contains("a%20-%3E%20b"));
        assert!(url.contains("count=5"));
    }

    #[test]
    fn build_search_url_with_offset() {
        let backend = WebBackend::new(&test_config()).unwrap();
        let url = backend.build_search_url("map", 10, 5);
        assert_eq!(
            url,
            "https://hoogle.haskell.org?mode=json&hoogle=map&start=11&count=5"
        );
    }
    ...
    fn build_search_url_type_signature() {
        let backend = WebBackend::new(&test_config()).unwrap();
        let url = backend.build_search_url("Ord k => k -> Map k v -> Maybe v", 0, 20);
    ...
    fn build_search_url_unicode() {
        let backend = WebBackend::new(&test_config()).unwrap();
        let url = backend.build_search_url("α -> β", 0, 5);
        assert!(url.contains("mode=json"));
    }
    ...
    fn build_search_url_empty() {
        let backend = WebBackend::new(&test_config()).unwrap();
        let url = backend.build_search_url("", 0, 10);
        assert_eq!(
            url,
